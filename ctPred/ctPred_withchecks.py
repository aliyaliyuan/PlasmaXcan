...
gen_data_p = '/home/aliya/Liver/newIbG/IbG_Hepatocyte_pericentral_merged.csv'
gen_data = pd.read_csv(gen_data_p)
gen_data

# Sanity checks
print("Shape:", gen_data.shape)
print("Columns[:5]:", gen_data.columns.tolist()[:5])
print("Chromo unique:", gen_data['chromo'].nunique())
print("Percentile stats:", gen_data['percentile'].describe())
...
# Check data distribution across chromosomes
print("\n" + "="*60)
print("Chromosome distribution:")
print(gen_data['chromo'].value_counts().sort_index())
print("="*60)

# Check each split
print("\nTrain set chromosomes:")
print(gen_data[gen_data['chromo'].isin(train_set)]['chromo'].value_counts())
print(f"Total train genes: {len(gen_data[gen_data['chromo'].isin(train_set)])}")

print("\nValidation set chromosomes:")
print(gen_data[gen_data['chromo'].isin(val_set)]['chromo'].value_counts())
print(f"Total validation genes: {len(gen_data[gen_data['chromo'].isin(val_set)])}")

print("\nTest set chromosomes:")
print(gen_data[gen_data['chromo'].isin(test_set)]['chromo'].value_counts())
print(f"Total test genes: {len(gen_data[gen_data['chromo'].isin(test_set)])}")
...
saved_path = '/home/aliya/Liver/ctPred_output/ctPred_model/scPred_Hepatocyte_pericentral4.pt'
...
def plot_prediction(model, test_features, test_labels):
    png_path = '/home/aliya/Liver/ctPred_output/png/scPred_Hepatocyte_pericentral4.png'
    model.eval()
...
scPred_reload = load_model('/home/aliya/Liver/ctPred_output/ctPred_model/scPred_Hepatocyte_pericentral4.pt', scPred)
